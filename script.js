...
// agent/ui/settings.js
var guiSettings = {
  killauraIntervalMs: 1e3
};
...
function updateKillaura(now) {
  ...
  if (now - _lastFireMs < guiSettings.killauraIntervalMs) return;
  try {
    const myX = scanData.myX, myY = scanData.myY;
    const own = scanData.ownCharacter;
...
function pushLog(message) {
  ...
  const MAX = 40;
  if (logMessages.length > MAX) {
    logMessages.splice(0, logMessages.length - MAX);
  }
}
...
// agent/ui/android.js
var _classId = 0;
function uniqueClassName(prefix) {
  return `com.revenge.ui.${prefix}.${++_classId}`;
}
function getClassLoader() {
  return {
    ...
    RelativeLayout_LayoutParams: Java.use("android.widget.RelativeLayout$LayoutParams"),
    ...
    LayerDrawable: Java.use("android.graphics.drawable.LayerDrawable"),
    ClipDrawable: Java.use("android.graphics.drawable.ClipDrawable"),
...
var Menu = class {
  ...
  addSlider(id, label, minVal, maxVal, defaultVal, onChange, opts = {}) {
    ...
    trackBg.setLayoutParams(tblp);
    trackBg.setBackground(makeRoundedDrawable(cl, THEME.track, 3, activity));
    const trackFill = cl.View.$new(activity);
    const fillLp = cl.FrameLayout_LayoutParams.$new(0, dp(activity, 6));
    fillLp.gravity = cl.Gravity.CENTER_VERTICAL.value | cl.Gravity.START.value;
    ...
    thumb.setSize(dp(activity, 16), dp(activity, 16));
    seekBar.setThumb(thumb);
    seekBar.setThumbOffset(dp(activity, 8));
  ...
  addLogButton() {
    ...
    const OpenLog = Java.registerClass({
      ...
      methods: {
        onClick() {
          that.#logOverlay.setVisibility(VIS_VISIBLE);
          that.updateLogView();
        }
      }
    });
  ...
  refreshMapOverlay() {
    ...
    Java.scheduleOnMainThread(() => {
      try {
        const bmp = cl.Bitmap.createBitmap(SIZE, SIZE, cl.BitmapConfig.ARGB_8888.value);
        const canvas = cl.Canvas.$new(bmp);
        const bgPaint = cl.Paint.$new();
        bgPaint.setColor(cl.Color.parseColor(THEME.mapBg));
        ...
        wallPaint.setColor(cl.Color.parseColor(THEME.mapWall));
        const movePaint = cl.Paint.$new();
        movePaint.setColor(cl.Color.parseColor(THEME.mapMove));
...
function tryStart(attempt) {
  try {
    ...
    if (!activity) {
      if (attempt < 80) setTimeout(() => tryStart(attempt + 1), 500);
      return;
    }
...
// agent/index.js
var _ACTIVE_MASK = FLAG_AIMBOT | FLAG_AUTODODGE | FLAG_ESP | FLAG_SPINNER | FLAG_KILLAURA;
